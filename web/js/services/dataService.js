/**
 * 数据管理服务模块 - services/dataService.js
 * 提供数据导入导出、备份恢复功能
 * 
 * 主要功能：
 * - 数据导出为.phone格式
...
// 处理文件导入事件
export function handleImportDataEvent(event) {
...
export async function clearAllData() {
    ...
    try {
        ...
        if (window.showCustomAlert) {
            await showCustomAlert("清空成功", "所有数据已清空。应用即将刷新。");
        }
        window.location.reload();
    } catch (error) {
        console.error("清空数据失败:", error);
        if (window.showCustomAlert) {
            showCustomAlert("清空失败", `发生错误: ${error.message}`);
        } else {
...
export async function getDataStats() {
    try {
        ...
        const stats = {
            ...
            userStickers: await db.userStickers.count(),
            worldBooks: await db.worldBooks.count(),
            personaPresets: await db.personaPresets.count(),
            totalMessages: 0,
            dataSize: 0 // 估算的数据大小
        };
